...
//  Game
//
//  Created by Muhammad Azka Subhan on 27/05/25.
//
...
// VARIABEL GLOBAL (GAME DAN TEKSTUR)
Spaceship player(2, 2); // Spaceship (User)
std::vector<Enemy> enemies; // Daftar Musuh
std::vector<Laser> lasers; // Daftar Laser
std::vector<Particle> particles; // Efek Partikel Ledakan
std::vector<Star> stars; // Bintang
GLuint starTexture, planetTexture, rockTexture, spaceshipTexture, sithTexture; // Tekstur
...
// IMPLEMENTASI STRUKTUR SPACESHIP (USER)
...
// Memindahkan Spaceship ke posisi baru
...
// Memperbarui posisi Spaceship dan animasi tabrakan
...
// IMPLEMENTASI STRUKTUR LASER
Laser::Laser(float px, float py, float pz) {
    x = px; y = py; z = pz;
    speed = 10.0f;
}

// IMPLEMENTASI STRUKTUR PARTIKEL
...
// MENGGAMBAR DAN MERENDER BINTANG
void drawStars() {
...
// MENGGAMBAR DAN MERENDER PLANET
void drawPlanet() {
    glPushMatrix();
...
// MENGGAMBAR DAN MERENDER PACESHIP (USER)
void drawSpaceship(const Spaceship& ship) {
    glPushMatrix();
    glTranslatef(ship.x, ship.y, ship.z);
    // Efek berkedip saat tabrakan
    if (ship.hitAnimation && fmod(ship.hitTimer * 10, 2) < 1) {
        glColor3f(1.0f, 0.0f, 0.0f);
    }
    glRotatef(ship.hitRotation, 0.0f, 0.0f, 1.0f);
    ...
    // BADAN UTAMA
    GLfloat body_material[] = {0.7f, 0.7f, 0.8f, 1.0f};
    glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, body_material);
    GLfloat body_specular[] = {0.9f, 0.9f, 1.0f, 1.0f};
...
// MENGGAMBAR DAN MERENDER MUSUH (KAPAL DAN ASTEROID)
void drawEnemy(const Enemy& enemy) {
    glPushMatrix();
    ...
    if (enemy.isRock) {
        ...
        // BADAN UTAMA KAPAL MUSUH
        glPushMatrix();
        glScalef(0.8f, 0.8f, 0.8f);
        glutSolidSphere(0.5f, 10, 10);
        glPopMatrix();
        // SAYAP KANAN KAPAL MUSUH
        glPushMatrix();
        glTranslatef(1.0f, 0.0f, 0.0f);
        glScalef(0.1f, 1.5f, 1.0f);
        glutSolidSphere(0.5f, 6, 6);
        glPopMatrix();
        // SAYAP KIRI KAPAL MUSUH
        glPushMatrix();
        glTranslatef(-1.0f, 0.0f, 0.0f);
        glScalef(0.1f, 1.5f, 1.0f);
        glutSolidSphere(0.5f, 6, 6);
...
// MENGGAMBAR DAN MERENDER PARTIKEL (HIT-EFFECT)
void drawParticles() {
    glDisable(GL_LIGHTING);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
void drawShadowPlane() {
    ...
    if (shadowsEnabled) {
        ...
        glRotatef(player.hitRotation, 0.0f, 0.0f, 1.0f);
        glScalef(player.size, player.size, player.size);
        glutSolidSphere(0.5f, 10, 10); // Bayangan Spaceship user
        glPopMatrix();
        ...
        for (const auto& enemy : enemies) {
            ...
            glScalef(enemy.size, enemy.size, enemy.size);
            if (enemy.isRock) {
                glutSolidDodecahedron(); // Bayangan Asteroid
            } else {
                glPushMatrix();
                glScalef(0.8f, 0.8f, 0.8f);
                glutSolidSphere(0.5f, 10, 10); // Bayangan Musuh
                glPopMatrix();
            }
            glPopMatrix();
        }
...
// UPDATE DAN SPAWN MUSUH
void updateEnemies() {
    for (size_t i = 0; i < enemies.size();) {
        Enemy& enemy = enemies[i];
        enemy.update(deltaTime);
        if (enemy.z > 10.0f) { // Menghapus musuh kalau melewati pemain
        ...
        if (dist < (player.size + enemy.size)) { // Menabrak user dan mengurangi nyawa user
            playerLives--;
            if (playerLives <= 0) {
                gameOver = true;
                gameRunning = false;
            ...
            enemies.erase(enemies.begin() + i);
            player.hitAnimation = true;
            continue;
        }
        i++;
    }
    if (enemies.size() < 5 && rand() % spawnDifficulty < 3) { // Spawn musuh
...
void updateLasers() {
    for (size_t i = 0; i < lasers.size();) {
        ...
        for (size_t j = 0; j < enemies.size(); j++) {
            ...
            float dist = sqrt(dx*dx + dy*dy + dz*dz);
            if (dist < (enemy.size + 0.5f) && !enemy.isRock) { // Mengenai musuh dan menambahkan skor
                score += 50;
                for (int k = 0; k < 20; k++) {
...
void drawUI() {
    ...
    if (gameOver) { // Tampilan layar game over
        ...
        float gameOverTextWidth = 0;
        for (char c : gameOverText) gameOverTextWidth += glutBitmapWidth(GLUT_BITMAP_TIMES_ROMAN_24, c);
        glRasterPos2f((windowWidth - gameOverTextWidth) / 2.0f, windowHeight / 2.0f + 80);
        for (char c : gameOverText) glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, c);
        std::string finalScoreText = "SKOR AKHIR: " + std::to_string(score);
        float finalScoreTextWidth = 0;
        for (char c : finalScoreText) finalScoreTextWidth += glutBitmapWidth(GLUT_BITMAP_HELVETICA_18, c);
        glRasterPos2f((windowWidth - finalScoreTextWidth) / 2.0f, windowHeight / 2.0f + 20);
        for (char c : finalScoreText) glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, c);
        std::string restartText = "Tekan R untuk Restart";
        float restartTextWidth = 0;
        for (char c : restartText) restartTextWidth += glutBitmapWidth(GLUT_BITMAP_HELVETICA_12, c);
        glRasterPos2f((windowWidth - restartTextWidth) / 2.0f, windowHeight / 2.0f - 30);
        for (char c : restartText) glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, c);
        }
    glColor3f(0.8f, 0.8f, 0.8f); // Teks kontrol di kanan atas layar
    glRasterPos2f(windowWidth - 400, windowHeight - 30);
    std::string controlText = "W A S D : Menggerakkan Kapal Angkasa | Space : Menembak";
    for (char c : controlText) glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, c);
    glRasterPos2f(windowWidth - 400, windowHeight - 50);
    std::string controlText2 = "Arrow Keys: Menggerakkan Kamera | Z: Shadow | C: View Dekat";
...
void keyboard(unsigned char key, int x, int y) {
    keys[key] = true;
    switch (key) {
        case 27: exit(0); break; // Keluar game
        case 'r': case 'R': // Restart game
            ...
            break;
        case 'z': case 'Z': shadowsEnabled = !shadowsEnabled; break; // Toggle bayangan
        case 'c': case 'C': followCamera = !followCamera; break; // Toggle kamera user
        case ' ': if (gameRunning) lasers.push_back(Laser(player.x, player.y, player.z)); break; // Menembak laser
    }
}

// INPUT KEYBOARD DILEPAS
void keyboardUp(unsigned char key, int x, int y) {
    keys[key] = false;
}

// INPUT KEYBOARD GERAKAN PEMAIN (WASD)
void handleKeys() {
...
// INPUT TOMBOL ARROW MENGGERAKKAN KAMERA
void specialKeys(int key, int x, int y) {
    float rotSpeed = 2.0f;
    switch (key) {
...
void timer(int value) {
    if (gameRunning) {
        ...
        if (fmod(time_var, 30.0f) < deltaTime && spawnDifficulty > 20) {
            spawnDifficulty -= 10;
            if (spawnDifficulty < 20) spawnDifficulty = 20;
        }
    }
    glutPostRedisplay();
